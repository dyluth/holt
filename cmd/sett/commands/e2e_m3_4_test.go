...
`
	...
	t.Log("Step 2: Starting Sett instance...")
	upCmd := &cobra.Command{}
	...
	verifyControllerCmd := exec.Command("docker", "ps",
		"--filter", fmt.Sprintf("name=sett-agent-%s-coder-controller", env.InstanceName),
		"--format", "{{.Names}}")
...
`
	...
	checkTraditionalCmd := exec.Command("docker", "ps",
		"--filter", fmt.Sprintf("name=sett-agent-%s-traditional-coder", env.InstanceName),
		"--format", "{{.Names}}")
	...
	checkControllerCmd := exec.Command("docker", "ps",
		"--filter", fmt.Sprintf("name=sett-agent-%s-controller-coder", env.InstanceName),
		"--format", "{{.Names}}")
