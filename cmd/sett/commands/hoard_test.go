...
func TestHoardCommand_Integration(t *testing.T) {
	t.Run("list mode - empty blackboard", func(t *testing.T) {
		...
		// Verify empty
		pattern := "sett:test-instance:artefact:*"
	...
	t.Run("JSON output format", func(t *testing.T) {
		...
		for iter.Next(ctx) {
			key := iter.Val()
			artefactID := strings.TrimPrefix(key, "sett:test-instance:artefact:")
	...
	t.Run("malformed artefact handling", func(t *testing.T) {
		...
		malformedKey := "sett:test-instance:artefact:malformed-123"
