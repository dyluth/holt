...
**Purpose**: Implement `sett forage` to initiate workflows and validate Phase 1 E2E pipeline
...
Implement the `sett forage` command to create GoalDefined artefacts and validate the complete Phase 1 event-driven pipeline from CLI through orchestrator to claim creation.

### **1.2. User story**

As a Sett user, I want to initiate a workflow by providing a high-level goal description so that the system can begin coordinating agents to accomplish that goal, and I want immediate feedback confirming the orchestrator received my request.

### **1.3. Success criteria**

* Running `sett forage --goal "hello world"` creates a GoalDefined artefact on the blackboard with correct structure
* The orchestrator detects the artefact within 200ms and creates a corresponding claim
* Running `sett forage --watch --goal "..."` validates the E2E pipeline by polling for claim creation and reporting success
...
* `sett watch` command structure exists as a stub with clear guidance for Phase 1 users
...
* **NOT in scope**: Real-time workflow monitoring beyond simple claim verification (Phase 2+ for `sett watch`)
...
**Justification:** No agents in Phase 1. Cub integration is Phase 2+.

### **2.4. CLI changes**

* **New/modified commands:**

**1. `sett forage [--name <instance>] [--watch] --goal "description"`**
...
* **Implementation:** Map Redis container port 6379 to host port (6379+), store in label `sett.redis.port`
...
* **Implementation:** Query Docker for containers with matching `sett.workspace.path` label
...
**Decision 5: Stub `sett watch` Command**
...
**Biggest Risks:**
1. **Port conflicts**: Multiple Sett instances or other services using 6379-638X
...
**Phase 4: `sett forage` Command (Days 5-6)**
- [ ] Create `cmd/sett/commands/forage.go`
...
**Phase 6: `sett watch` Stub Command (Day 8)**
- [ ] Create `cmd/sett/commands/watch.go`
  - [ ] Define command structure with Cobra
  - [ ] Add `--name` flag (for consistency)
  - [ ] Implement stub behavior:
    - [ ] Print informational message
    - [ ] Suggest using `sett forage --watch` for Phase 1
    - [ ] Exit with code 0
...
- [ ] Update `cmd/sett/commands/README.md` or create forage-specific docs
...
* **Integration tests:**
  * `cmd/sett/commands/forage_integration_test.go` (requires Docker):
...
* **E2E tests (full system):**
  * `cmd/sett/commands/e2e_test.go`:
    * `TestE2E_Phase1_Heartbeat`:
      1. Run `sett init` in temp directory
      2. Run `sett up`
      3. Run `sett forage --watch --goal "hello world"`
      4. Verify:
         - Artefact created
    ...
    * `TestE2E_ForageWatch_OrchestatorDown`:
      ...
      2. Run `sett forage --watch --goal "test"`
...
* **Privilege requirements:**
  * Docker socket access (same as M1.4 `sett up`)
...
* `sett forage` can be added to existing systems without migration
* Redis port exposure is additive to M1.4 (doesn't break existing functionality)

**Breaking change in M1.4 modification:**
* Existing `sett up` behavior changes slightly (adds port mapping)
...
* **Docker requirements:**
  ...
  * Adds one label (`sett.redis.port`) to Redis containers
...
- [ ] `sett forage --goal "..."` creates GoalDefined artefact correctly
- [ ] `sett forage --watch --goal "..."` detects claim creation within timeout
- [ ] `sett watch` stub prints informational message and exits
...
- [ ] E2E test passes: `sett init && sett up && sett forage --watch --goal "hello world"`
...
* **Scenario:** User runs `sett forage` outside Git repository
...
* **Scenario:** User runs forage but no Sett instances exist
* **Detection:** Docker query returns empty list
* **Response:** Fail with startup instructions
* **Error Message:**
  ```
  Error: no Sett instances found
...
* **Error Message:**
  ...
  Specify which instance to use:
    sett forage --name dev --goal "your goal"
    sett forage --name test --goal "your goal"
  ```
...
* **Scenario:** Container missing `sett.redis.port` label
...
* **Scenario:** User runs `sett forage` without --goal
...
* **Error Message:**
  ...
  Example:
    sett forage --goal "Build a web scraper for news articles"
  ```
...
* **Error Message:**
  ...
  Check orchestrator status:
    ...
    docker logs sett-orchestrator-prod

  Check artefact was created:
    sett hoard --name prod
  ```
...
* **Scenario:** User runs `sett forage` in multiple terminals
...
* **Scenario:** Two `sett up` commands run simultaneously
...
* **Handling:** Port allocation fails, sett up returns error
* **Error Message:**
  ```
  ...
  Stop unused instances:
    sett list
    sett down --name <unused-instance>
  ```
* **Implementation:** Check range exhaustion before bind attempt

**Edge Case 7: Rapid forage + down cycle**
* **Scenario:** User creates artefact then immediately runs `sett down`
...
⏭️ **Q10: How to implement real-time `sett watch`?**
...
- [x] M1.3 (CLI Project Initialization) complete - sett init works
- [x] M1.4 (CLI Lifecycle Management) complete - sett up/down work
...
Next steps:
  - Run 'sett watch' to monitor progress (Phase 2)
  - Run 'sett hoard' to view all artefacts
...
Please commit changes before running sett forage
```
...
3. **Port conflicts**: Check port allocation errors in `sett up` logs
...
**New CLI commands:**
* `sett forage --goal "..."` - Create workflow
* `sett forage --watch --goal "..."` - Create and validate
* `sett watch` - Stub for Phase 2

**User guides to update:**
* `README.md`: Add Phase 1 E2E workflow section
* `cmd/sett/commands/forage.md`: Complete usage guide
...
Usage:
  sett forage [--name <instance>] [--watch] --goal "description"
...
* **Issue:** "No Sett instances found"
  * **Check:** Run `sett list` to verify
  * **Fix:** Start instance with `sett up`

* **Issue:** "Multiple instances found"
  * **Check:** Run `sett list` to see instances
...
* **Issue:** "Timeout waiting for claim"
  ...
  * **Fix:** Check orchestrator logs: `docker logs sett-orchestrator-<instance>`
  * **Verify:** Run `sett hoard` to confirm artefact was created
...
```go
// Example GoalDefined artefact created by sett forage
...
func FindNextAvailablePort(ctx context.Context, cli *client.Client) (int, error) {
    ...
    filters.Add("label", "sett.component=redis")
    ...
    for _, container := range containers {
        if portStr, ok := container.Labels["sett.redis.port"]; ok {
...
Check artefact was created:
  sett hoard`, int(timeout.Seconds()))
