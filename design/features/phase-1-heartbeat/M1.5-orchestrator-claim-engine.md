...
As a Sett operator, I want the orchestrator to automatically detect when new artefacts appear on the blackboard and create corresponding claims so that the system's event-driven coordination infrastructure is proven to work, enabling agent integration in Phase 2.

### **1.3. Success criteria**

* When the CLI creates a GoalDefined artefact via `sett forage`, the orchestrator detects it within 100ms and creates a corresponding claim in `pending_review` status
...
* **NOT in scope**: Agent registry loading from `sett.yml` (YAGNI - no bidding in Phase 1)
...
* **NOT in scope**: Integration with `sett up` command (that's M1.6)
...
* **Usage patterns:**
  * Subscribes to `sett:{instance_name}:artefact_events` for real-time artefact notifications
  * Publishes to `sett:{instance_name}:claim_events` when new claims are created
...
* **New/modified configurations (sett.yml):** No configuration file loading required in M1.5
...
**Justification:** No agents exist in Phase 1. The orchestrator publishes to `claim_events`, but no cubs are listening yet. This proves the infrastructure works for Phase 2 integration.
...
**Justification:** The orchestrator is a background service. CLI commands (`sett forage` in M1.6) will trigger it indirectly, but M1.5 doesn't modify the CLI itself.

**Note:** M1.6 will integrate the orchestrator container into `sett up`, but that's outside M1.5's scope.
...
* **Why This Approach:** Aligns with Sett's event-driven architecture and keeps orchestrator stateless
...
* **Rationale:** Agent registry not needed in Phase 1 (no bidding), so no need to parse `sett.yml`
...
- [ ] Implement environment variable loading (`SETT_INSTANCE_NAME`, `REDIS_URL`)
...
- [ ] Copy binary to `/usr/local/bin/sett-orchestrator`
- [ ] Set `ENTRYPOINT ["/usr/local/bin/sett-orchestrator"]`
...
* **E2E tests (sett tests):**
  * Deferred to **M1.6** - requires full system integration with `sett up` and `sett forage`
...
* **Component boundaries:**
  * Orchestrator does NOT execute tools (that's Agent Cub's job)
  * Orchestrator does NOT parse `sett.yml` (that's CLI's job in M1.4, orchestrator's job in Phase 2)
...
* **Tight coupling avoided:**
  ...
  * No direct dependencies on CLI or Agent Cub packages
...
- [ ] Dockerfile for `sett-orchestrator` builds successfully and produces working image
...
✅ **Q1: Should orchestrator load `sett.yml` for agent registry?**
...
✅ **Q3: Should M1.5 integrate orchestrator into `sett up`?**
...
* **Problem:** Orchestrator starts with empty `SETT_INSTANCE_NAME`, creates invalid Redis keys
...
* However, in M1.4 integration (M1.6), orchestrator container can be stopped: `docker stop sett-orchestrator-{instance}`

**Rollback procedure:**
* **If orchestrator has bugs:** Stop container, revert to busybox placeholder from M1.4
* **Steps:**
  1. `sett down --name {instance}`
  2. Revert `sett up` command to use busybox orchestrator
  3. `sett up --name {instance}`
...
* **Issue:** Claims not being created
  ...
  * **Check:** Review orchestrator logs (`docker logs sett-orchestrator-{instance}`)

* **Issue:** Orchestrator exits immediately
  * **Check:** Verify environment variables are set (`SETT_INSTANCE_NAME`, `REDIS_URL`)
...
import (
    ...
    "github.com/yourusername/sett/internal/orchestrator"
    "github.com/yourusername/sett/pkg/blackboard"
...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o sett-orchestrator ./cmd/orchestrator
...
RUN adduser -D -u 1000 sett

# Copy binary from builder
COPY --from=builder /build/sett-orchestrator /usr/local/bin/sett-orchestrator
...
# Set entrypoint
ENTRYPOINT ["/usr/local/bin/sett-orchestrator"]

# Document required environment variables
ENV SETT_INSTANCE_NAME=""
ENV REDIS_URL=""
```
...
**M1.6 will update `sett up` to:**

1. Build orchestrator image: `docker build -t sett-orchestrator:latest -f Dockerfile .`
2. Replace busybox placeholder with real orchestrator:
   ```go
   // M1.6 changes to cmd/sett/commands/up.go:

   // Build orchestrator image
   buildCtx := docker.BuildContext{
       Dockerfile: "Dockerfile",
       Tags:       []string{"sett-orchestrator:latest"},
   }
   ...
   containerResp, err := client.ContainerCreate(ctx, &container.Config{
       Image: "sett-orchestrator:latest",  // Changed from busybox
       Env: []string{
