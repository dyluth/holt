...
As a Holt user, after running a workflow, I need a way to easily see what artefacts were produced. I want to be able to get a quick overview of everything on the blackboard in a clean table format, and if one item looks interesting, I want to be able to retrieve all of its data for detailed inspection with a single command.

### **1.3. Success criteria**

* A user can run `holt hoard` (with auto-inferred instance) and see a clean, readable table of all artefacts with ID, Type, Produced By, and Payload summary columns
* A user can run `holt hoard <artefact-id>` and receive the complete artefact object as pretty-printed JSON for detailed inspection
* A user can run `holt hoard --output=json` and receive all artefacts as a JSON array suitable for programmatic processing
...
* Real-time streaming (--watch flag) - use `holt watch` command instead
...
* Redis SCAN with pattern `holt:{instance}:artefact:*` - used in list mode
...
```
Artefacts for instance 'holt-test-2024-10-15':

┌──────────────────────────────────────┬─────────────────┬─────────────┬────────────────────────────────────────────────────────────┐
│ ID                                   │ TYPE            │ PRODUCED BY │ PAYLOAD                                                    │
├──────────────────────────────────────┼─────────────────┼─────────────┼────────────────────────────────────────────────────────────┤
│ 25e839bd-2fe9-4c11-84da-e92b603766a8 │ GoalDefined     │ user        │ hello-from-holt.txt                                        │
...
*Empty state:*
```
No artefacts found for instance 'holt-test-2024-10-15'
```
...
**Phase 1: Command infrastructure (cmd/holt/commands/hoard.go)**
...
2. Use Redis SCAN with pattern `holt:{instance}:artefact:*`
...
* Not designed for real-time monitoring of 10,000+ artefacts (use `holt watch` for that)
...
**Integration tests:**
* `cmd/holt/commands/hoard_test.go`: Test with miniredis
...
* Add to Phase 2 E2E test suite:
  ...
  ARTEFACT_ID=$(holt hoard --output=json | jq -r '.[0].id')
  holt hoard $ARTEFACT_ID
  # Assert: Valid JSON object

  # Test not found
  holt hoard 00000000-0000-0000-0000-000000000000
...
* `cmd/holt/commands/hoard.go` - CLI command wiring
...
* Existing `holt hoard` stub is being promoted to full command (non-breaking)
...
* Error: `✗ Multiple instances found. Use --name to specify: holt hoard --name <instance>`
* Exit code: 1
...
* Warning: `⚠️  Skipping malformed artefact: key=holt:instance:artefact:xxx (parse error)`
...
* ✅ Redis key pattern: Use SCAN with holt:{instance}:artefact:* (approved)
...
* Update `holt hoard --help` with full description
...
holt hoard --output=json | jq -r '.[].id' | head -1
```
