...
**Scope**: Agent pup context assembly, thread tracking, git integration, filtered context chain
...
As a Holt developer, I need agents to receive complete historical context when executing work, so that LLM-driven agents can make informed decisions based on the full chain of previous work, and code-generating agents can produce git commits that are properly validated.
...
**Git Integration:**
* When tool output has `artefact_type: "CodeCommit"`, pup validates the commit hash exists
* Validation uses `git cat-file -e <commit-hash>` to check commit existence
* If commit doesn't exist, pup creates Failure artefact with descriptive error
...
* **NOT in scope**: Workspace state management - pre-flight check in `holt up` command
...
**Rationale:** Orchestrator creates claims and publishes events. Context assembly is entirely pup-side.
...
2. **Context Assembly Algorithm (from agent-pup.md):**
   ```
   Algorithm:
...
**New files:**
- `internal/pup/context.go` - Context assembly implementation
- `internal/pup/context_test.go` - Unit tests for assembly algorithm
- `internal/pup/git.go` - Git validation helpers
- `internal/pup/git_test.go` - Git validation tests

**Modified files:**
- `internal/pup/executor.go` - Call context assembly, validate git commits
- `internal/pup/contract.go` - Document context_chain population
...
- Document recommended commit message format (not enforced by pup)

**Rationale:** Context assembly is purely pup-side. CLI only needs documentation updates.
...
9. **No Workspace Management:**
   - **Decision:** Pup does NOT check workspace state, checkout commits, or manage branches
   - **Rationale:** Pre-flight check in `holt up`, agent script manages git
   - **Separation of concerns:** Pup executes and validates, script manages workspace

10. **Commit Message Format (Documentation Only):**
    - **Decision:** Document recommended format, do NOT enforce in pup
    - **Rationale:** Agent script responsibility, pup doesn't parse commits
    - **Format:** `[holt-agent: {role}] {summary}\n\nClaim-ID: {claim-id}`
...
1. **[Pup]** Create `internal/pup/context.go`:
...
2. **[Pup]** Implement BFS traversal loop:
...
3. **[Pup]** Implement filtering and sorting:
...
4. **[Pup]** Add unit tests for context assembly:
...
5. **[Pup]** Create `internal/pup/git.go`:
...
6. **[Pup]** Add unit tests for git validation:
...
7. **[Pup]** Update `executor.go::prepareToolInput()`:
...
8. **[Pup]** Update `executor.go::createResultArtefact()`:
...
9. **[Pup]** Update failure handling for git validation:
...
10. **[Pup]** Create `internal/pup/context_integration_test.go`:
...
11. **[Pup]** Create git validation integration test:
...
12. **[Pup]** Create executor integration test with context:
...
Location: `internal/pup/context_test.go`, `internal/pup/git_test.go`
...
Location: `internal/pup/context_integration_test.go`
...
Location: `cmd/holt/commands/e2e_context_test.go`
...
* No workspace state changes by pup
...
8. **Workspace not a git repository:**
   ...
   - **Work executor:** Create Failure artefact
   - **Note:** Should be caught by pre-flight check in `holt up`
...
5. Run `holt up` (context_chain will be empty array again)
...
While not enforced by the pup, agent scripts should follow this format for consistency and traceability:

```
[holt-agent: {agent-role}] {summary from ToolOutput}
...
* [ ] I understand the BFS algorithm from agent-pup.md
