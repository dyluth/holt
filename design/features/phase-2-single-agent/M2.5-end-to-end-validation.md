...
As a Sett developer preparing to release Phase 2, I need a comprehensive E2E test suite that validates the complete workflow from `sett forage` through agent execution to CodeCommit artefact creation, proves that critical failure scenarios are handled correctly with appropriate error messages and Failure artefacts, and provides clear documentation so new users can get started and agent developers can build their own agents with confidence.

### **1.3. Success criteria**

**End-to-End Happy Path:**
* Test validates complete workflow: `sett forage --goal "create hello.txt"` → orchestrator creates claim → git agent bids → git agent wins grant → agent executes → creates file in workspace → commits to Git → returns commit hash → CodeCommit artefact created on blackboard
...
* Test runs in complete isolation using t.TempDir() with fresh Git repo and sett.yml

**Failure Scenario Validation (Critical Guardrails):**
* **Dirty Git Workspace Test**: Creates uncommitted changes → executes `sett up` → asserts command fails with clear error message → no containers launched
...
* **Startup Performance Test**: Measures `sett up` duration → logs measured time → fails if > 10 seconds
...
* **docs/troubleshooting.md (Moderate Detail)**: Common error messages → likely causes → solutions → essential debug commands (sett logs, docker ps, sett hoard)
...
* **NOT in scope**: Human interaction commands (`sett questions`, `sett answer`) - Phase 4
...
**New/modified configurations (sett.yml):** No changes to schema
...
**New/modified logic:** No changes to cub code

**Changes to tool execution contract:** No changes

**Rationale:** M2.5 validates existing cub functionality (claim watching, bidding, context assembly, tool execution, git validation) works correctly. All cub logic for Phase 2 was implemented in M2.1-M2.4.
...
**New test files:**
* `cmd/sett/commands/e2e_phase2_test.go` - Complete Phase 2 E2E test suite
* `cmd/sett/commands/e2e_performance_test.go` - Performance validation tests
* `cmd/sett/commands/e2e_failures_test.go` - Failure scenario tests

**Test isolation infrastructure:**
* Each test function uses `t.TempDir()` for isolated temporary directory
* Test setup: creates Git repo, sett.yml, agent scripts programmatically
...
* Helper functions for common E2E setup tasks (create test sett.yml, initialize Git repo, wait for containers)
...
├── Dockerfile          # Multi-stage build: Go cub + git tools
...
5. Execute: `git commit -m "[sett-agent: git-agent] Created <filename>\n\nClaim-ID: <claim-id>"`
...
* [ ] Write `Dockerfile` (multi-stage: build cub + install git)
...
* [ ] Create `cmd/sett/commands/e2e_phase2_test.go`
* [ ] Implement test setup: `t.TempDir()`, Git repo init, sett.yml creation
* [ ] Implement test step 1: `sett up` launches orchestrator + git agent
* [ ] Implement test step 2: `sett forage --goal "hello.txt"` creates GoalDefined artefact
...
* [ ] Implement test cleanup: `sett down` stops containers

**Step 3: Implement Failure Scenario Tests**

* [ ] Create `cmd/sett/commands/e2e_failures_test.go`
* [ ] Implement `TestE2E_DirtyGitWorkspace`: uncommitted changes → `sett up` fails with clear error
...
* [ ] Create `cmd/sett/commands/e2e_performance_test.go`
* [ ] Implement `TestPerformance_Startup`: measure `sett up` duration, assert < 10s
...
* [ ] Section 1: "Sett won't start" - Redis connection issues, port conflicts
...
* [ ] Section 1: Project Summary (what is Sett? 2-3 paragraphs)
* [ ] Section 2: Quick Start (clone → build → run example-git-agent workflow)
* [ ] Section 3: Core Concepts (Blackboard, Orchestrator, Cub, Claims, Artefacts)
...
* [ ] Manual smoke test: fresh clone → `sett init` → `sett up` → `sett forage` → verify CodeCommit
...
- Cub startup: < 1 second (implicit - not separately tested)
- `sett up` startup: < 10 seconds (explicit test)
...
- Cub configuration parsing (`internal/cub/config_test.go`)
- Context assembly algorithm (`internal/cub/context_test.go`)
- Git validation logic (`internal/cub/git_test.go`)
- Tool contract serialization (`internal/cub/contract_test.go`)
...
- Cub engine lifecycle with real containers

**E2E tests (PRIMARY FOCUS OF M2.5):**

**File: `cmd/sett/commands/e2e_phase2_test.go`**
...
func TestE2E_Phase2_HappyPath(t *testing.T) {
    // Setup: t.TempDir(), Git init, sett.yml creation
    // Step 1: sett up (verify containers running)
    // Step 2: sett forage --goal "hello.txt"
...
**File: `cmd/sett/commands/e2e_failures_test.go`**

```go
// TestE2E_DirtyGitWorkspace verifies sett up fails with dirty workspace
...
func TestPerformance_ContextAssembly(t *testing.T) {
    ...
    // Measure: time for cub to assemble context
    // Log: measured duration
...
# Run only E2E tests
go test -v -tags=integration ./cmd/sett/commands/e2e_*.go

# Run specific E2E test
go test -v -tags=integration ./cmd/sett/commands -run TestE2E_Phase2_HappyPath
```
...
func TestE2E_Something(t *testing.T) {
    ...
    // Create sett.yml in temp dir
    os.WriteFile(filepath.Join(tmpDir, "sett.yml"), []byte(config), 0644)
...
- Failure artefacts created by cub on errors
...
E2E tests verify that `sett up` correctly rejects dirty Git workspaces, preventing accidental data loss or corruption.
...
Test agents use simple shell scripts with no API keys or credentials. Documentation emphasizes that real agents should use environment variables for secrets (existing pattern from sett.yml).
...
* [ ] `cmd/sett/commands/e2e_phase2_test.go` created with happy path test
* [ ] `cmd/sett/commands/e2e_failures_test.go` created with 3 failure scenario tests
* [ ] `cmd/sett/commands/e2e_performance_test.go` created with 4 performance tests
...
* [ ] Manual smoke test: fresh repo clone → build → sett up → sett forage → verify result
...
**Scenario:** User runs `sett up` in repository with uncommitted changes

**Expected behavior:**
1. `sett up` command detects uncommitted changes via `git status --porcelain`
2. Command fails immediately with error: "Git workspace is dirty. Please commit or stash changes before running sett up."
...
**Expected behavior:**
1. Cub executes tool subprocess with timeout
2. Process exits with code != 0
3. Cub captures stderr output
4. Cub creates Failure artefact with structural_type="Failure"
5. Failure artefact payload contains: exit code, stderr output, error message
6. Cub updates claim status to "terminated"
...
**Expected behavior:**
1. Cub executes tool subprocess successfully (exit code 0)
2. Cub attempts to parse stdout JSON
...
**Expected behavior:**
1. Cub parses tool output successfully (valid JSON)
2. Cub sees artefact_type="CodeCommit"
3. Cub executes `git cat-file -e <hash>` for validation
4. Git command fails (hash doesn't exist)
5. Cub creates Failure artefact with structural_type="Failure"
...
4. Cub loses Redis connection
5. Cub health check endpoint returns 503
6. Cub logs error, pending operations fail
7. User can inspect logs via `sett logs <component>`
...
5. User can inspect logs via `sett logs <agent>`
6. User must manually intervene (fix issue, restart via `sett down && sett up`)
...
**Current state:** Cub processes one claim at a time via work queue channel (buffer size 1). Safe by design.

**Consideration 3: Blackboard Concurrent Access**

Multiple components (CLI, orchestrator, cub) access Redis concurrently. Redis handles concurrency natively via single-threaded command processing.
...
**Expected behavior:** Redis accepts payload (default max string length 512MB). Cub creates artefact successfully.
...
**Expected behavior:** Cub kills subprocess after timeout, creates Failure artefact with "execution timeout" error
...
**Scenario:** User runs `sett up --name instance1`, then `sett up --name instance2` in same directory

**Expected behavior:** Second `sett up` fails with error "workspace already in use by instance1" (workspace locking implemented in M1.4)
...
**Context:** E2E tests will have common patterns (setup Git repo, create sett.yml, wait for containers, query blackboard).
...
**Context:** Users might need to debug Docker networking, volume mounts, container logs beyond `sett logs`.

**Options:**
- A: Only document `sett` commands (keeps users in Sett's interface)
...
3. Write `Dockerfile` that builds cub and installs Git
...
2. Implement `TestE2E_Phase2_HappyPath` incrementally:
   ...
   - Add step 1 (sett up) and verify containers running
   - Add step 2 (sett forage) and verify artefact created
...
1. Start with `TestPerformance_Startup` (measures sett up)
...
5. Have someone unfamiliar with Sett review docs for clarity
...
func TestE2E(t *testing.T) {
    exec.Command("sett", "up").Run() // Pollutes project directory!
}
```
...
func TestE2E(t *testing.T) {
    ...
    os.Chdir(tmpDir)
    defer os.Chdir(originalDir)
    exec.Command("sett", "up").Run() // Isolated!
}
```
...
* [ ] Cub's git validation logic is implemented (M2.4)
...
* [ ] Each test creates sett.yml programmatically (no reliance on project files)
* [ ] Each test uses unique instance names to avoid Docker conflicts
* [ ] Each test has proper cleanup (defer statements for sett down)
...
* [ ] Documentation can be followed by someone unfamiliar with Sett
...
- `sett logs` command provides debugging visibility
...
- How to safely stop sett if workflow is stuck
- How to clean up Docker containers manually if `sett down` fails
...
* [ ] All Phase 2 success criteria from MILESTONES.md are satisfied
* [ ] Manual smoke test completed: clone → build → sett up → forage → verify
