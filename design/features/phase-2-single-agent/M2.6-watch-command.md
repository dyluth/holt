...
**Purpose**: Implement comprehensive real-time monitoring of Sett workflows via CLI
...
As a Sett user, I want to run `sett forage --watch --goal "my task"` and immediately see the complete lifecycle of my workflow—from goal creation through agent bidding to final execution—in a continuous stream without needing to run additional commands. This allows me to monitor progress, debug issues, and understand system behavior in real-time.

### **1.3. Success criteria**

* A user can run `sett watch` (with auto-inferred instance) or `sett watch --name instance-name` and see a continuous stream of all workflow events (artefact creations, claims, bids, grants) until they press Ctrl+C.
* A user can run `sett forage --watch --goal "task"` and after the initial claim is created, the command automatically transitions into full watch mode, streaming all subsequent activity without exiting.
* A user can run `sett watch --output=json` and receive line-delimited JSON events suitable for parsing by external tools or scripts.
...
* `sett:{instance}:workflow_events` - New channel for bid and grant events
...
1. **`sett watch` - Complete rewrite from stub**
   * Flags:
...
2. **`sett forage` - Enhanced --watch behavior**
   * Modified behavior when `--watch` flag is true:
     ...
     * Call same streaming logic as `sett watch` command
...
**Phase 4: Watch command implementation (cmd/sett/commands/)**
...
**Phase 5: Forage --watch integration (cmd/sett/commands/)**
...
3. Add E2E test: `sett forage --watch` full workflow validation
4. Add E2E test: `sett watch --output=json` parsing validation
...
1. **Full workflow watch test**:
   ```bash
   sett forage --watch --goal "test-file.txt" &
...
2. **JSON output parsing test**:
   ```bash
   sett watch --output=json > events.json &
...
* Bids are stored in `sett:{instance}:claim:{id}:bids` hash (persistent, queryable)
...
* **Cub**: Unchanged (bids remain blackboard operation)
...
* Error: `✗ Multiple instances found. Use --name to specify: sett watch --name <instance>`
* Exit code: 1
...
* Mitigation: User can query current claim state with `sett hoard` if needed
...
* Check Redis Pub/Sub: `redis-cli PUBSUB CHANNELS sett:*:workflow_events`
* Check active subscribers: `redis-cli PUBSUB NUMSUB sett:<instance>:workflow_events`
* Verify event publishing: `redis-cli SUBSCRIBE sett:<instance>:workflow_events` (manual monitoring)
...
3. If watch command has issues, users can continue using `sett hoard` for state inspection
...
* Update `sett watch --help` with full command description and examples
* Update `sett forage --help` to clarify --watch behavior
* Add `sett watch` examples to README or user guide
...
sett watch --name prod-instance

# Export events as JSON for analysis
sett watch --output=json > workflow-events.jsonl

# Submit goal and watch full workflow
sett forage --watch --goal "Add user authentication"
```
