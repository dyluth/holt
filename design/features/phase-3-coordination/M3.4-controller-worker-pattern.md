...
As a Holt user, I want to scale my agents horizontally so that:
...
1. **Controller configuration**: `holt.yml` supports `mode: "controller"` with nested `worker:` config
...
4. **Worker execution**: Worker executes claim via `pup --execute-claim <claim_id>` and exits
...
9. **Backward compatibility**: Existing `holt.yml` without `mode` continues working unchanged
10. **Configuration validation**: `holt up` validates controller configuration
...
type WorkerState struct {
    ...
    ContainerName string    // holt-{instance}-{agent}-worker-{claim}
...
func (e *Engine) LaunchWorker(ctx context.Context, claim *blackboard.Claim, agentName string, agent config.Agent) error {
    ...
    containerName := fmt.Sprintf("holt-%s-%s-worker-%s", e.instanceName, agentName, shortClaimID)
    ...
    containerConfig := &container.Config{
        ...
        Env: []string{
            fmt.Sprintf("HOLT_INSTANCE=%s", e.instanceName),
            ...
            // to trigger worker mode in the Pup
        },
    }

    hostConfig := &container.HostConfig{
        NetworkMode: container.NetworkMode(fmt.Sprintf("holt-%s", e.instanceName)),
...
**Major enhancement**: Pup gains execute-claim mode and controller-only mode.

#### **2.3.1. Command-line flag handling**

**File**: `cmd/pup/main.go`
...
func main() {
    ...
    // Load configuration (reads HOLT_MODE env var)
    cfg := loadConfig()

    // M3.4: Mode decision tree
    // 1. If HOLT_MODE=controller (set by instance manager) → controller mode
    // 2. Else if --execute-claim <id> (set by orchestrator) → worker mode
    // 3. Else (no HOLT_MODE, no flag) → traditional mode
    if cfg.Mode == "controller" {
        log.Printf("[Pup] Starting in controller mode (bidder-only)")
        runControllerMode(cfg)
    } else if *executeClaimID != "" {
        log.Printf("[Pup] Starting in worker mode for claim %s", *executeClaimID)
        runWorkerMode(cfg, *executeClaimID)
    } else {
        // Traditional agent mode (M3.3)
        log.Printf("[Pup] Starting in traditional agent mode")
...
**File**: `internal/pup/controller.go` (new file)
...
**File**: `internal/pup/worker.go` (new file)
...
func (m *Manager) startAgentContainer(ctx context.Context, agentName string, agent config.Agent) error {
    containerName := fmt.Sprintf("holt-%s-%s", m.instanceName, agentName)

    // Both controller and traditional agents run pup with no flags
    cmd := []string{"/app/pup"}

    // Build environment variables
    env := []string{
        fmt.Sprintf("HOLT_INSTANCE=%s", m.instanceName),
        fmt.Sprintf("HOLT_AGENT_NAME=%s", agentName),
        fmt.Sprintf("HOLT_AGENT_ROLE=%s", agent.Role),
        fmt.Sprintf("REDIS_ADDR=redis:6379"),
    }

    // M3.4: Set HOLT_MODE only for controllers
    // Traditional agents get no HOLT_MODE (will default to traditional mode)
    // Workers (launched by orchestrator) get no HOLT_MODE (use --execute-claim flag)
    if agent.Mode == "controller" {
...
1. **Explicit mode configuration**:
   - **Decision**: Use `mode: "controller"` in holt.yml
...
2. **Command-line claim delivery**:
   - **Decision**: `pup --execute-claim <claim_id>`
...
6. **Worker naming convention**:
   - **Decision**: `holt-{instance}-{agent_name}-worker-{short_claim_uuid}`
...
2. **Worker orphaning**:
   ...
   - **Mitigation**: M3.5 restart resilience, manual cleanup via `holt down`
...
5. **Network race conditions**:
   ...
   - **Mitigation**: Worker pup retries Redis connection on startup
...
5. **[Config]** Update example `holt.yml` with controller example

**Phase 2: Pup Controller Mode** (1 PR)

6. **[Pup]** Add `--execute-claim` flag to `cmd/pup/main.go`
7. **[Pup]** Create `internal/pup/controller.go` with controller mode logic
8. **[Pup]** Implement bidder-only mode (subscribe, bid, never execute)
9. **[Pup]** Add mode detection from env var `HOLT_MODE`
10. **[Pup]** Write unit tests for controller bidding logic

**Phase 3: Pup Worker Mode** (1 PR)

11. **[Pup]** Create `internal/pup/worker.go` with worker mode logic
12. **[Pup]** Implement execute-only mode (fetch claim, execute, exit)
13. **[Pup]** Add claim fetching and validation
14. **[Pup]** Write unit tests for worker execution flow
...
23. **[Instance]** Update `startAgentContainer()` to pass `HOLT_MODE` env var
...
26. **[Tests]** Create `cmd/holt/commands/e2e_m3_4_test.go`:
...
1. **Controller-worker basic flow**:
   ...
   Steps:
     1. holt up with controller configuration
     2. holt forage --goal "test goal"
...
3. **Worker failure handling**:
   ...
   Steps:
     ...
     2. holt forage --goal "failing goal"
...
- **Pup**: Adds controller mode (bidder-only) and worker mode (execute-only)
...
**Data validation:**
- Worker configuration validated at `holt up`
...
- [ ] Example holt.yml demonstrates controller configuration
...
7. **Controller config missing worker block**:
   - **Behavior**: Config validation fails at `holt up`
...
3. **Orchestrator restart with active workers**:
   ...
   - **Recovery**: Manual cleanup via `holt down`
...
- **Recovery**: `holt down && holt up` restarts controller
...
2. ✅ **Worker grant delivery**: Command-line arg `pup --execute-claim <claim_id>`
...
6. ✅ **Worker naming**: `holt-{instance}-{agent_name}-worker-{short_claim_uuid}`
...
3. Then pup mode detection and flags
...
- [ ] Pup --execute-claim flag added
- [ ] Pup controller mode (bidder-only) implemented
- [ ] Pup worker mode (execute-only) implemented
...
```json
// Worker launched
{"level":"info","component":"orchestrator","event":"worker_launching","container_name":"holt-inst-coder-worker-abc12345","claim_id":"uuid","role":"Coder","timestamp":"2025-10-20T00:00:00Z"}
...
- List active workers:
  ```bash
  docker ps --filter "name=holt-*-worker-*"
  ```
- Check worker logs:
  ```bash
  docker logs holt-{instance}-{agent}-worker-{claim}
  ```
- Monitor worker count:
  ```bash
  holt logs orchestrator | grep "worker_launched\|worker_cleanup"
  ```
...
2. **Config changes**: Remove `mode` and `worker` from holt.yml
...
1. **Orchestrator crash with active workers**:
   ...
   - **Recovery**: `holt down` cleans up all containers including orphans

2. **Worker containers orphaned**:
   - Orchestrator loses tracking state (restart)
   - **Recovery**: `holt down && holt up` removes all workers
...
1. **Common issues**:
   ...
   - "Worker containers orphaned" → Run `holt down` to cleanup
...
2. **Debugging commands**:
   ...
   holt logs orchestrator | grep worker

   # Count active workers per role
   docker ps --filter "name=holt-*-worker-*" | wc -l
   ```
...
- [x] Pup modes are unambiguous (controller vs worker vs traditional)
...
- [ ] Example holt.yml with controller config provided and tested
