...
As a Sett user, I want to define multiple specialized agents in my sett.yml (e.g., a reviewer agent, a coder agent, and a test agent) so that when I run `sett forage --goal "implement feature X"`, all agents can evaluate the goal and bid appropriately, with the orchestrator waiting for all bids before granting the claim to the most appropriate agent. This ensures that every agent has an opportunity to participate in the workflow and that the system makes deterministic, reproducible decisions about which agent should handle each piece of work.

### **1.3. Success criteria**

1. **Multi-agent startup**: `sett up` with 3 agents defined in sett.yml successfully starts all containers in parallel and validates health checks before reporting success
2. **Full consensus bidding**: `sett forage --goal "test"` triggers all 3 agents to evaluate and submit bids (exclusive, review, ignore), and the orchestrator waits for all 3 bids before proceeding
...
4. **Complete audit trail**: `sett hoard` shows all bids submitted (with agent names and bid types) and the grant decision with clear reasoning in logs
...
- **Dynamic agent registration** (Future): Agents must be defined in sett.yml at startup
...
**Current Solution:** Hardcoded heuristic in `internal/cub/engine.go:handleClaimEvent()` - agents automatically bid "ignore" on artefacts where `artefact.ProducedByRole == agent.AgentRole`. This prevents agents from bidding on their own outputs.
...
- **Agent registry tracking**: No new Redis keys needed - agent list comes from sett.yml loaded at startup
...
1. **Agent Registry Management**:
   - Load complete agent list from sett.yml at startup
...
2. **Full Consensus Bidding Model** (Major Enhancement):
   ...
   //    - Store in Redis: sett:{instance}:claim:{uuid}:bids
...
- The cub's execution behavior is unchanged
...
1. **Enhanced `sett up`**:
   ```
   Usage: sett up [--name <instance>] [--force]
...
2. **Enhanced Output**:
   ...
   ✓ Started orchestrator: sett-orchestrator-my-instance
   ✓ Started agent containers (parallel):
     • sett-agent-my-instance-coder-agent (running, healthy)
     • sett-agent-my-instance-reviewer-agent (running, healthy)
     • sett-agent-my-instance-test-agent (running, healthy)
   ✓ Instance 'my-instance' started successfully (3 agents ready)
   ```

3. **Agent Container Orchestration**:
   - For each agent in sett.yml:
     - Inject environment variables:
       - Existing: SETT_INSTANCE_NAME, SETT_AGENT_NAME, REDIS_URL, SETT_AGENT_ROLE
       - New: SETT_BIDDING_STRATEGY (from sett.yml)
...
2. **Configuration-Based Bidding Strategy**:
   - **Decision**: Bidding strategy declared in sett.yml, not LLM-driven
...
4. **[Cub]** Add `SETT_BIDDING_STRATEGY` to cub config loading in `internal/cub/config.go`
5. **[Cub]** Implement fail-fast validation for missing/invalid bidding strategy
...
6. **[Orchestrator]** Create `internal/orchestrator/consensus.go` with full consensus model:
   ...
   - Agent registry management (load from sett.yml)
...
**Phase 3: Agent Cub Bidding**

10. **[Cub]** Update `internal/cub/watcher.go` bid submission:
...
11. **[Cub]** Update cub tests to use configurable bidding strategy

**Phase 4: CLI Multi-Agent Launch**

12. **[CLI]** Enhance `cmd/sett/commands/up.go` for parallel agent launch:
    - Create `launchAgentContainersParallel()` function
    - Use goroutines + sync.WaitGroup for parallel start
    - Inject `SETT_BIDDING_STRATEGY` environment variable
...
15. **[Agents]** Create `agents/example-reviewer-agent/`:
    - Dockerfile with cub entrypoint
    - `run.sh` that always outputs Review artefact with `{}`
    - sett.yml snippet with `bidding_strategy: review`
16. **[Agents]** Update `agents/example-git-agent/` to use `bidding_strategy: exclusive` in sett.yml
17. **[Agents]** Create `agents/example-test-agent/`:
    - Dockerfile with cub entrypoint
    - `run.sh` that always bids ignore (outputs nothing)
    - sett.yml snippet with `bidding_strategy: ignore`

**Phase 6: Testing & Validation**

18. **[Tests]** Create `cmd/sett/commands/e2e_multi_agent_test.go`:
...
23. **[Docs]** Create example sett.yml with multiple agents
...
- **Startup time**: `sett up` completes in <15s with 5 agents
...
2. **Agent bidding**:
   - `internal/cub/cub_integration_test.go`: Real Redis + cub
   - Cub receives claim, submits configured bid type
   - Verify bid appears in Redis
   - Test all bid types (review, claim, exclusive, ignore)

3. **CLI agent launch**:
   - `cmd/sett/commands/up_integration_test.go`: Docker + Redis
...
1. **Three-agent happy path** (`cmd/sett/commands/e2e_multi_agent_test.go`):
   ```
...
- All bids stored immutably in `sett:{instance}:claim:{uuid}:bids` hash
...
2. **Workspace access**: Each agent's workspace mount mode (ro/rw) enforced per sett.yml
3. **Resource limits**: Agent resource constraints (CPU, memory) respected from sett.yml
4. **No new attack surfaces**: All communication via existing Redis channels (no new network exposure)

**Credentials and secrets:**
- Environment variables (including SETT_BIDDING_STRATEGY) passed securely via Docker
...
1. **sett.yml schema change** (additive):
   - New required field: `bidding_strategy` for each agent
   - **Breaking change**: Existing sett.yml files must add this field
...
2. **Environment variables** (additive):
   - New: `SETT_BIDDING_STRATEGY` passed to cub
...
- CLI commands unchanged (sett up, forage, down, etc.)
- Agent cub behavior unchanged for granted claims
...
4. **Missing SETT_BIDDING_STRATEGY environment variable**:
   - **Behavior**: Cub exits with error on startup (fail-fast)
   - **Detection**: Immediate at cub initialization
   - **Recovery**: Fix sett.yml, restart with `sett down && sett up`
   - **Logging**: "SETT_BIDDING_STRATEGY environment variable is required"

5. **Invalid bidding_strategy value in sett.yml**:
   - **Behavior**: Config validation fails during `sett up` (fail-fast)
   - **Detection**: Config parsing stage
   - **Recovery**: Fix sett.yml value, retry `sett up`
...
6. **Agent container fails health check**:
   - **Behavior**: `sett up` fails, rolls back all containers
...
7. **Parallel container launch failure (one agent fails to start)**:
   - **Behavior**: Stop all agents, rollback, `sett up` fails
...
8. **Health check timeout (agents don't become healthy within 30s)**:
   - **Behavior**: `sett up` fails with timeout error
...
1. **Multiple bids from same agent**:
   - **Possibility**: Agent cub submits duplicate bids due to retry logic
...
4. **Grant notification race**:
   - **Possibility**: Orchestrator publishes grant while cub is restarting
   - **Protection**: Agent cub polls claim status (doesn't rely solely on Pub/Sub)
...
**Edge case: No agents defined in sett.yml**:
- **Behavior**: Config validation fails, `sett up` fails
- **Detection**: Config parsing stage
- **Recovery**: Add at least one agent to sett.yml
- **Logging**: "No agents defined in sett.yml, at least one agent is required"
...
1. ✅ **Bidding strategy**: Configuration-based (sett.yml field)
...
3. Then agent cub bidding (simple change)
...
6. **Missing environment variable handling**:
   - Check `SETT_BIDDING_STRATEGY` at startup
...
- [x] New data structures are backward compatible (sett.yml additive)
...
- [ ] Agent cub bidding tested with all four bid types
...
- [ ] Example sett.yml with multiple agents provided
...
- Agent cub `/healthz` remains unchanged (Redis connectivity)
...
- Redis CLI: Inspect bid hashes for any claim
  ```
  redis-cli HGETALL sett:my-instance:claim:abc123:bids
  ```
...
- Rollback to Phase 2: Use single-agent sett.yml
- **Rollback procedure**:
  1. `sett down` (stop current instance)
  2. Edit sett.yml (remove extra agents)
  3. `sett up` (restart with single agent)

**Rollback procedure for failed deployment:**

1. **Detect failure**: `sett up` reports error or health checks fail
2. **Automatic rollback**: CLI stops all containers automatically
3. **Manual cleanup**: If needed, `docker ps -a | grep sett-agent` and remove
4. **Fix configuration**: Edit sett.yml to correct error
5. **Retry**: Run `sett up` again
...
1. **README.md**:
   ...
   - Add example sett.yml with 3 agents
...
1. **Common issues**:
   ...
   - "Invalid bidding_strategy" → Check sett.yml spelling
...
2. **Debugging commands**:
   ...
   # View agent logs
   sett logs coder-agent

   # Inspect bids in Redis
   redis-cli -h localhost -p 6379
   > HGETALL sett:my-instance:claim:abc123:bids

   # Check orchestrator consensus logs
   docker logs sett-orchestrator-my-instance | grep consensus
   ```
