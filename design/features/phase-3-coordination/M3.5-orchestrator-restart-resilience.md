...
As a Sett user running production workflows, I want the orchestrator to automatically recover from crashes so that:
...
This enables reliable, unattended operation of Sett in production environments where orchestrator restarts (planned or unplanned) should not break workflows.
...
9. **Backward compatibility**: M3.5 orchestrator handles clean M3.4 → M3.5 upgrade path (requires `sett down` then `sett up`)
10. **Minimal performance overhead**: State persistence operations complete within 10ms per write

### **1.4. Non-goals**

- **Dynamic configuration reload**: Changing `sett.yml` during orchestrator runtime (config still loaded at startup only)
...
grant_queue_position   int                     // Position in FIFO queue (for ordering)
...
#### **2.1.2. New Redis keys**

```
sett:{instance_name}:grant_queue:{role}   # ZSET: score=paused_at timestamp, member=claim_id
                                          # Maintains FIFO queue per role for max_concurrent pausing
```

**Rationale**: Using a ZSET allows efficient FIFO ordering (by timestamp) and atomic dequeue operations.
...
func (e *Engine) PauseGrantForQueue(ctx context.Context, claim *blackboard.Claim, agentName string, role string) error {
    ...
    queueKey := fmt.Sprintf("sett:%s:grant_queue:%s", e.instanceName, role)
...
**No changes required**. Agent cubs are stateless and receive grants via Pub/Sub. The orchestrator handles all recovery logic.

### **2.4. CLI changes**

**Minor enhancement**: `sett up` should detect stale locks and provide clear messaging.
...
func detectStaleLock(ctx context.Context, cli *client.Client, instanceName string) (bool, error) {
    lockKey := fmt.Sprintf("sett:%s:lock", instanceName)
...
23. **[CLI]** Implement `detectStaleLock()` in `sett up` command
...
26. **[Tests]** Create `cmd/sett/commands/e2e_m3_5_test.go`
...
- Upgrade path: `sett down` (M3.4) → `sett up` (M3.5)
...
- List active claims:
  ```bash
  sett status --name instance  # Shows claims in recovery, phases, queue
  ```
- Check recovery logs:
  ```bash
  sett logs orchestrator | grep recovery
  ```
- Monitor persistence:
  ```bash
  sett logs orchestrator | grep "phase_transition\\|grant_queue"
  ```
...
2. **Procedure**:
   - Stop M3.5 orchestrator: `sett down`
   - Rollback orchestrator binary/image to M3.4
   - Start M3.4 orchestrator: `sett up`
3. **Impact**: In-flight claims may be stuck (M3.4 has no recovery logic)
4. **Workaround**: Manually terminate stuck claims: `sett terminate <claim_id>` (future CLI command)

**Upgrade path M3.4 → M3.5:**

1. **Stop M3.4 instance**: `sett down --name instance`
2. **Upgrade orchestrator binary/image** to M3.5
3. **Start M3.5 instance**: `sett up --name instance`
...
2. **Debugging commands**:
   ...
   sett logs orchestrator | grep recovery_complete

   # List orphaned workers
   docker ps -a --filter "label=sett.instance=instance" --filter "label=sett.run_id!=current-run-id"

   # Check grant queue
   redis-cli ZRANGE sett:instance:grant_queue:role 0 -1 WITHSCORES

   # Verify claim phase state
   redis-cli HGETALL sett:instance:claim:uuid | grep phase_

   # Monitor recovery metrics
   sett logs orchestrator | grep -E "recovery_|claim_recovered|orphan_"
   ```
