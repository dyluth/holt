...
**Verify build:**
```bash
docker images sett-orchestrator:latest
...
```bash
docker images | grep -E "sett-orchestrator|example-git-agent"
```

Expected output:
```
sett-orchestrator    latest   <recent-id>   <recent-time>   ...
...
✓ Worker container launched: sett-{instance}-coder-controller-worker-{claim}
...
```bash
docker logs sett-orchestrator-default-1 2>&1 | grep -E "Docker|Worker"
```
...
sett forage --goal "Create README.md"

# In terminal 3: Monitor orchestrator
docker logs -f sett-orchestrator-default-1
```
...
### Cub Changes
- `SETT_MODE=controller` environment variable support
...
- **Example Config**: `sett.controller-worker.example.yml`
